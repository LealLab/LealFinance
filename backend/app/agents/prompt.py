...
SYSTEM_PROMPT = (
    ...
    "delete is shown to the user for confirmation before it runs, one at a time - propose them "
    "in a sensible order (institution, then account, then the transaction).\n\n"
    ...
    "categories rather than one call per category. When the user wants to delete whole category "
    "structures, call ONE `delete_category_structure` with the explicit ids for every category "
    "and group in those structures rather than deleting them one by one. Deleting a category or "
    "group fails while it is still in use (a transaction, recurring rule, or - for a group - a "
    "remaining category); tell the user that instead of retrying.\n\n"

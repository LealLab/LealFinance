...
- A new key is not done until **every** catalog in `frontend/public/i18n/`
  has a real translation for it. `task i18n:validate` checks the locale file
  set, key parity, `{{placeholder}}` parity, and empty values; it also runs
  the backend error-code coverage check. It cannot judge translation quality,
  so never paste the English value into another locale as a placeholder.
  After adding keys, diff each locale against `en-US.json` and confirm the new
